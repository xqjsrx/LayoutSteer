...
# [LayoutSteer shim] tf>=4.50 PreTrainedModel 不再继承 GenerationMixin
...
# [LayoutSteer shim] 干预探针全局挂载点（VendorHookProbe.attach 设置）
_LS_PROBE = None
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # [LayoutSteer shim] ΔK 干预回调（post-RoPE / post-cache, [B, KV, seq, d]）
        if _LS_PROBE is not None:
            key_states = _LS_PROBE.maybe_edit(query_states, key_states, self.layer_idx)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ##########spatial embeddings##########
            # [LayoutSteer shim] tf>=4.50 的 generate 预建空 DynamicCache，
            # len() 恒为层数，须改用 get_seq_length()==0 判首个 prefill
            _is_prefill = (past_key_values is None
                           or (past_key_values.get_seq_length() == 0
                               if hasattr(past_key_values, "get_seq_length")
                               else len(past_key_values) == 0))
            if _is_prefill:
        ...
        if use_cache:  # kept for BC (cache positions)
            # [LayoutSteer shim] tf>=4.50 的 generate 传入的已是 DynamicCache，
            # 对其调 from_legacy_cache 会按 legacy 元组迭代出 None 而崩溃
        ...
        # print(attention_mask.shape);0/0
        # pdb.set_trace()
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                # [LayoutSteer shim] tf>=4.57 移除 get_max_length；
                # get_max_cache_shape 对无界 cache 返回 -1/None，统一归一为 None
                if hasattr(past_key_values, "get_max_length"):
                    _max_len = past_key_values.get_max_length()
                else:
                    _max_len = past_key_values.get_max_cache_shape()
                    if _max_len is not None and _max_len < 0:
                        _max_len = None
                max_cache_length = (
                    torch.tensor(_max_len, device=input_ids.device)
