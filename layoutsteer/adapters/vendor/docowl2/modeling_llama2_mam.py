...
# [LayoutSteer shim] 干预探针挂载点（ScoreAddProbe.attach 设置）
_LS_PROBE = None
...
from transformers.generation import GenerationMixin as _LSGenerationMixin


class LlamaForCausalLM(LlamaPreTrainedModel, _LSGenerationMixin):
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # [LayoutSteer shim] Tier C: softmax 前给目标图像 token 的 score 列加 δ
        if _LS_PROBE is not None:
            attn_weights = _LS_PROBE.maybe_add_scores(attn_weights, self._ls_layer_idx)
